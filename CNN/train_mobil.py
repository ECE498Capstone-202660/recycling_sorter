...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, device, num_epochs=70):
    best_acc = 0.0
...
if __name__ == '__main__':
    ...
    model = MobileNetWithMass(num_classes=5).to(device)
    criterion = nn.CrossEntropyLoss(weight=class_weights)
    optimizer = optim.Adam(model.parameters(), lr=0.0005)
    scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=5)

    history = train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, device)
    plot_loss_curves(history, "MobileNetV2 + Mass")

    model.load_state_dict(torch.load("mobile_model.pth"))
    preds, labels = evaluate_model(model, test_loader, criterion, device)
    plot_confusion_matrix(labels, preds, CATEGORIES, "MobileNetV2 + Mass")
